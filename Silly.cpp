...
List c_jaeger_model_V3(NumericVector t, NumericVector X, List parms_list) {
	...
	//double K_13           = parms_list["K_13"];
        ...
        double repression = parms_list["repression"];
	...
	// Repression
        p_13_4 = repression * p_13_4;
        p_13_5 = repression * p_13_5;
        v1[3]  = repression * v1[3];
    
    
	//Synthesis Rates
	  NumericMatrix rho(5,3);
    
	  rho(0,0) = eta_leaf * X[5] / v1[0];
	  rho(0,1) = 0;
	  rho(0,2) = 0;
	...
	// v1[0] = eta_leaf * t;
	  ...
	  Vout[0] = v_35S[0] + (rho(0,0)*v1[0] + rho(0,1)*v2[0] + rho(0,2)*v3[0]) * (1-mutants[0]) - (delta[0] * X[0]);
